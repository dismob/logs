...
"""
Shared fixtures for the `logs` plugin's tests.

This plugin is a standalone Git submodule (dismob/logs repo), so its tests
live here rather than in the main bot repo, so they travel with the plugin
regardless of which bot it's used in.
